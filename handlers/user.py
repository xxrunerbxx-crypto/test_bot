...
router = Router()
DEBUG_LOG_PATH = Path(__file__).resolve().parent.parent / "debug-b1c8fb.log"
...
async def main_menu(message: Message, command: CommandObject, state: FSMContext):
    ...
    _debug_log(
        "pre-fix",
        "H4",
        "handlers/user.py:main_menu",
        "Start command received",
        {"has_args": bool(args), "user_id": message.from_user.id},
    )
    # endregion
...
async def show_calendar(callback: CallbackQuery, state: FSMContext):
    ...
    _debug_log(
        "pre-fix",
        "H4",
        "handlers/user.py:show_calendar",
        "Start booking opened",
        {"master_id_present": bool(master_id), "master_id": master_id},
    )
    # endregion
...
async def finish_booking(message: Message, state: FSMContext, bot: Bot):
    ...
    _debug_log(
        "pre-fix",
        "H5",
        "handlers/user.py:finish_booking",
        "Booking confirmation step",
        {
            "has_master_id": bool(data.get("master_id")),
            "has_slot_id": bool(data.get("slot_id")),
