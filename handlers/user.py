...
async def main_menu(event, state: FSMContext = None):
    ...
    if isinstance(event, Message):
        ...
        try:
            # Пытаемся удалить сообщение (особенно если там было фото услуг)
...
async def show_calendar(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        parse_mode="HTML", reply_markup=generate_calendar(now.year, now.month)
    )
...
async def choose_time(callback: CallbackQuery, state: FSMContext):
    ...
    if not slots:
        return await callback.answer("❌ Нет свободных мест.", show_alert=True)
    
    builder = InlineKeyboardBuilder()
    for s_id, s_time in slots:
        builder.add(InlineKeyboardButton(text=s_time, callback_data=f"slot_{s_id}_{s_time}"))
    builder.adjust(3).row(InlineKeyboardButton(text="⬅️ Назад", callback_data="start_booking"))
    
    await callback.message.edit_text(f"⏰ Время на <b>{date}</b>:", 
...
async def ask_phone(message: Message, state: FSMContext):
    ...
    await message.answer("📞 Отправьте номер через кнопку или вручную:", reply_markup=inline.phone_kb())
...
async def finish_booking(message: Message, state: FSMContext, bot: Bot):
    ...
    if not phone:
        return await message.answer("❌ Введите корректный номер.")
    ...
    try:
        ...
        db.create_booking(user.id, data['slot_id'], data['name'], phone, f"{data['date']} {data['time']}", str(job_id) if job_id else "no_reminder")
        ...
        # Оповещение только в КАНАЛ
        admin_msg = (f"🆕 <b>НОВАЯ ЗАПИСЬ!</b>\n\n👤 {data['name']}\n📞 <code>{phone}</code>\n"
                     f"📱 {username}\n📅 {data['date']} в {data['time']}")
        if CHANNEL_ID:
